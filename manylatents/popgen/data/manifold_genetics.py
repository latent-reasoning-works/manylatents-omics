...
Inheritance from PrecomputedDataModule:
    This class extends PrecomputedDataModule to reuse the channel-like pattern for
    loading multiple related files. The key differences:
    - Pre-split data: manifold-genetics produces separate fit/transform outputs
    - Sample ID joining: files are merged by sample_id column
    - Domain metadata: admixture ratios, colormaps, geographic labels
"""
...
class ManifoldGeneticsDataModule(PrecomputedDataModule):
    """
    PyTorch Lightning DataModule for manifold-genetics standardized outputs.

    Extends PrecomputedDataModule to handle the manifold-genetics output format
    with pre-split train/test files and sample_id-based joining.
    ...
    def __init__(
        ...
        # Initialize parent with a placeholder - we override setup() to load real data
        # This satisfies PrecomputedDataModule's validation while allowing us to
        # handle the pre-split manifold-genetics format
        super().__init__(
            data=np.zeros((1, 1)),  # Placeholder, replaced in setup()
            batch_size=batch_size,
            num_workers=num_workers,
            mode=mode,
        )

        # Store manifold-genetics specific paths
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        Overrides PrecomputedDataModule.setup() to handle manifold-genetics
        pre-split format with separate train/test files joined by sample_id.
        ...
        if self.hparams.mode == "split":
            ...
            raise ValueError(f"Invalid mode '{self.hparams.mode}'. Use 'split' or 'full'.")

    def train_dataloader(self) -> DataLoader:
        """Return training DataLoader with custom collate function."""
        return DataLoader(
            self.train_dataset,
    ...
    # Compatibility properties for direct attribute access

    @property
    def mode(self):
        """Mode for backward compatibility (delegates to hparams)."""
        return self.hparams.mode

    @property
    def batch_size(self):
        """Batch size for backward compatibility (delegates to hparams)."""
        return self.hparams.batch_size

    @property
    def num_workers(self):
        """Num workers for backward compatibility (delegates to hparams)."""
        return self.hparams.num_workers

    # Domain-specific accessors that delegate to the underlying dataset

    @property
    def admixture_ratios(self):
        """Get admixture ratios from train dataset (for metrics)."""
        if self.train_dataset is None:
            raise RuntimeError("Call setup() before accessing admixture_ratios")
        return self.train_dataset.admixture_ratios

    @property
    def colormap(self):
        """Get colormap from train dataset."""
        if self.train_dataset is None:
            raise RuntimeError("Call setup() before accessing colormap")
        return self.train_dataset.colormap

    @property
    def population_label(self):
        """Get population labels from train dataset (for metrics)."""
        if self.train_dataset is None:
            raise RuntimeError("Call setup() before accessing population_label")
        return self.train_dataset.population_label

    def get_labels(self, label_col: Optional[str] = None) -> np.ndarray:
        """Get integer-encoded labels from train dataset."""
        if self.train_dataset is None:
            raise RuntimeError("Call setup() before accessing labels")
        return self.train_dataset.get_labels(label_col)

    def get_label_names(self, label_col: Optional[str] = None) -> np.ndarray:
        """Get string labels from train dataset."""
        if self.train_dataset is None:
            raise RuntimeError("Call setup() before accessing label names")
        return self.train_dataset.get_label_names(label_col)

    def get_tensor(self) -> torch.Tensor:
        """Return data tensor from train dataset for LatentModule compatibility."""
        if self.train_dataset is None:
            raise RuntimeError("Call setup() before accessing tensor")
        return torch.from_numpy(self.train_dataset.data_array).float()
